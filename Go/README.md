...
- ✅ **系统状态查询** (`/start` → 📊 系统状态)：实时监控 CPU、内存、磁盘及运行时间
- ✅ **立即执行维护** (`/start` → 🔧 立即维护)：支持核心维护、规则更新及全量维护
- ✅ **定时任务管理** (`/start` → ⚙️ 定时设置)：可视化管理定时任务，支持状态持久化
- ✅ **查看服务日志** (`/start` → 📋 查看日志)：实时获取服务运行日志
- ✅ **安全重启** (`/start` → ♻️ 重启 VPS)：带确认机制的安全重启功能
- ✅ **自动维护**：默认每周日 04:00 执行全量维护（系统更新 + 规则更新 + 自动重启）

## 安全特性

- **权限控制**：严格的 Chat ID 验证，仅允许授权管理员操作
- **输入验证**：防止命令注入和非法字符输入
- **脚本安全**：执行维护脚本前进行存在性检查和权限验证
- **敏感信息保护**：Token 等敏感信息通过环境变量传递，不硬编码

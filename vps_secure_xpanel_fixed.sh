#!/bin/bash
# -----------------------------------------------------------------------------------------
# VPS 代理服务端口检测与防火墙配置脚本（V3.8.2 修复最终版）
# 兼容 xeefei X-Panel / X-UI / Xray / Sing-box
#
# 🩵 更新日志:
# V3.8.2-FIXED
#   ✅ [严重] 修复 setup_firewall 日志污染导致变量识别错误的问题
#   ✅ [重构] 提取 SSH 端口检测为独立函数，消除重复代码
#   ✅ [优化] 增强主程序逻辑，安装防火墙后进行二次状态确认
# -----------------------------------------------------------------------------------------
...
setup_fail2ban() {
    ...
    *) echo "无效输入，使用默认激进模式"; FAIL2BAN_MODE="激进 (Aggressive)"; bantime="1h"; maxretry="3"; findtime="10m" ;;
    esac
...
main() {
    ...
    if [ "$firewall_type" = "none" ]; then
        setup_firewall # 直接执行，不捕获输出，避免变量污染
        # 3. 安装后再次检测状态
        firewall_type=$(detect_firewall)
    fi

#!/bin/bash
# -----------------------------------------------------------------------------------------
# VPS 代理服务端口检测与防火墙配置脚本（V3.8.4 终极修复版）
# 兼容 xeefei X-Panel / X-UI / Xray / Sing-box
#
# 🔥 更新日志:
# V3.8.4 - [终极修复版 - 恢复 Emoji]
#   ✅ 修复 Fail2Ban 日志路径：自动检测 Debian/RHEL 系统
#   ✅ 修复偏执模式 bantime：12小时而非1小时
#   ✅ 修复 Sing-box 重复检测：移除通用配置文件检查
#   ✅ 增强端口去重：过滤非数字和空值
#   ✅ 添加防火墙重置警告提示
#   ✅ 改进配置文件遍历：使用 nullglob 防止通配符失败
#   ✅ 恢复 Emoji：更友好的视觉体验
#   ✅ 增强错误处理和用户提示
# -----------------------------------------------------------------------------------------
...
setup_fail2ban() {
    ...
    local banaction
    local logpath
    banaction=$(detect_banaction "$firewall_type")
...
remove_unused_rules() {
    ...
    local ports_array
    ports_array=($(echo "$ports_to_keep" | tr ' ' '\n' | grep -E '^[0-9]+$' | sort -un))

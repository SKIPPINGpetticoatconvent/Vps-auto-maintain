...
| 命令 | 描述 | 参数 |
| :--- | :--- | :--- |
| `/update_xray` | 更新 Xray 到最新版本 | 无 |
| `/update_sb` | 更新 Sing-box 到最新版本 | 无 |
| `/maintain_core` | 执行核心系统维护（包管理） | 无 |
| `/maintain_rules` | 执行规则维护（Xray 规则更新） | 无 |

#### 管理命令
| 命令 | 描述 | 参数 |
| :--- | :--- | :--- |
| `/logs` | 查看系统日志 | 可选：行数（默认 20） |
| `/set_schedule <cron表达式>` | 设置定时维护计划 | 必填：Cron 表达式 |
...
| 🔄 系统更新 | 执行核心系统维护 |
| 🌍 规则更新 | 执行 Xray 规则更新 |
| 🚀 更新 Xray | 更新 Xray 到最新版本 |
| 📦 更新 Sing-box | 更新 Sing-box 到最新版本 |
| 🔙 返回主菜单 | 返回主菜单界面 |

### 2.1 核心功能
| ID | 功能模块 | 原始逻辑 (Shell/Python) | Rust 实现方案 | 实现状态 |
| :--- | :--- | :--- | :--- | :--- |
| F-01 | **Bot 交互** | `python-telegram-bot` 处理 `/start`, `/status` 等命令 | 使用 `teloxide` crate 实现异步 Bot 交互 | ✅ 已完成 |
| F-02 | **系统监控** | `uptime`, `date` 命令输出 | 使用 `sysinfo` crate 获取结构化的 CPU/内存/运行时间数据 | ✅ 已完成 |
| F-03 | **核心维护** | 调用 `apt update`, `xray up`, `sb up` | 使用 `std::process::Command` 封装系统包管理和特定软件更新命令 | ✅ 已完成 |
| F-04 | **规则更新** | 调用 `xray up dat` | 同上，封装为独立的维护任务 | ✅ 已完成 |
| F-05 | **定时任务** | `apscheduler` (每周日 04:00) | 使用 `tokio-cron-scheduler` 实现进程内调度，不再依赖外部 crontab | ✅ 已完成 |
| F-06 | **日志查看** | `journalctl` 查询 | 读取自身日志文件或通过 `systemd` API (或继续调用 `journalctl` 命令) | ✅ 已完成 |
| F-07 | **系统控制** | `reboot` 命令 | 调用系统重启命令，需处理权限检查 | ✅ 已完成 |

### 2.2 已实现的新功能
| ID | 功能模块 | 描述 | 实现状态 |
| :--- | :--- | :--- | :--- |
| NF-01 | **特定软件更新** | 支持 Xray 和 Sing-box 的独立更新命令 | ✅ 已完成 |
| NF-02 | **细粒度维护任务** | 将维护分为 Core（系统核心）和 Rules（规则）两个独立任务 | ✅ 已完成 |
| NF-03 | **日志查看** | 实现系统日志查看功能，支持指定行数 | ✅ 已完成 |
| NF-04 | **调度任务持久化与动态管理** | 调度计划持久化存储，支持运行时动态更新 | ✅ 已完成 |
| NF-05 | **Inline Keyboards 交互菜单** | 实现直观的按钮式交互界面，提升用户体验 | ✅ 已完成 |
